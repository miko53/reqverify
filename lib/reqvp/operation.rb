...
module Reqv
  ...
  class OperationHelp < Operation
    ...
    def print_help
      puts("#{Reqv.exe_name} commands:")
      puts("#{Reqv.exe_name} create_project <project_name> <folder> <filename>\tcreate a new project in the given folder" \
          ' with the given filename')
      puts("#{Reqv.exe_name} add_doc <project_file> raw <doc_name> <doc_filename>")
      puts("#{Reqv.exe_name} add_doc <project_file> import <doc_name> handler <import_plugin> <doc_filename>" \
        ' <doc_yaml_file,optional>')
      puts("#{Reqv.exe_name} add_plugin_rule <project_file> <doc_name> <rule_name> <rule_value> <rule_type, optional>")
      puts("#{Reqv.exe_name} add_relationships <project_file> <relation name> <doc_list> covered-by <doc_list>")
      puts("#{Reqv.exe_name} add_derived_name <project_file> <derived_regular exp>")
    end
  end
